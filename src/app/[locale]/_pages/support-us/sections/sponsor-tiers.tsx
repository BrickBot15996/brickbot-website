...
export function SponsorshipTierDesktop({
  ...
  const t = useTranslations("SponsorshipTierData");
  return (
        ...
        >
          {tier.benefits.map((benefit, index) => {
            return (
              <li
                key={index}
                className={`flex items-start before:content-[''] before:w-[1rem] before:h-[1rem] before:bg-[url(https://cdn.brickbot.ro/icons/green-checkmark.png)] before:bg-contain before:bg-no-repeat before:mr-[0.5rem] before:flex-shrink-0 before:mt-[0.4rem] mx-[1rem]`}
              >
                {benefit}
              </li>
            );
